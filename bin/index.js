...
const get$1 = key => {
  ...
  if (key && typeof key === 'object') {
    return Object.fromEntries(Object.entries(key).filter(([key]) => config[key] !== undefined).map(([key]) => [key, config[key]]));
  }
  if (key) {
    return config[key];
  }
...
const selectConfigSchema = z.object(Object.fromEntries(configSchema.keyof().options.map(key => [key, z.boolean().optional()])));

const set$1 = newConfigData => {
  const configPath = resolveCurrentDir('config.json');
  const config = get$1();
...
const getInstallationString = (manager, packages) => {
  switch (manager) {
    case 'npm':
...
const installDependencies = (manager, packages) => {
...
const setupFunctionsDictionary = {
  ...
  eslint: async manager => {
    ...
    installDependencies(manager, ['eslint', '@eslint/js', 'eslint-plugin-react', 'eslint-plugin-react-hooks', 'eslint-plugin-simple-import-sort', 'globals', 'typescript-eslint']);
  },
...
const init = {
  ...
  options: [['-p, --prettier', 'setup prettier'], ['-e, --eslint', 'setup eslint'], ['-t, --typescript', 'setup typescript'], ['-s, --stylelint', 'setup stylelint'], ['-m, --manager <npm | yarn | pnpm>', 'package manager that will be used to install dependencies']],
  schema: initSchema,
  isDefault: true,
  action: options => {
    ...
    if (!manager) {
      return logger.error("Manager didn't specified. Please, set it up with `set` command or use -m or --manager option.");
    }
    ...
    initOptions.forEach(([key, value]) => {
      if (value) {
        setupFunctionsDictionary[key](manager);
      }
    });
  }
};
...
const parseCliError = (error, argv, declaredOptions) => {
  const option = declaredOptions.map(getOptionKeys).find(({
    long,
    short
  }) => {
...
Object.values(commands).map(command => {
  ...
  currentCommand.action(options => {
    const {
      ...
      data,
      error
    } = command.schema.safeParse(options);
    if (success) {
